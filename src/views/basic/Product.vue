  ...
  <div class="product">
    ...
    <el-tabs v-model="activeName">
      <el-tab-pane label="产品管理" name="first">
        <el-button type="primary" @click="productAdd" style="marginBottom:15px">添加产品</el-button>
                ...
                <div class="product_card">
                  <el-card class="product_card_one">
                    <div class="rolesexpand">
                      <div>
                        <el-tag size="mini" type="info">产品编号</el-tag>
                        {{ scope.row.code }}
                      </div>
                      <div>
                        <el-tag size="mini" type="info">产品ID</el-tag>
                        {{ scope.row.id }}
                      </div>
                      <div>
                        <el-tag size="mini" type="info">产品名称</el-tag>
                        {{ scope.row.name }}
                      </div>
                      <div>
                      ...
                      <div>
                        <el-tag size="mini" type="info">产品单位</el-tag>
                        {{ scope.row.categoryVO.name }}
                      </div>
                      <div>
            ...
            <el-table-column align="center" prop="updateTime" label="修改时间">
              <template slot-scope="scope">
                ...
                <el-tag size="small" type="info" v-else>
                  {{ scope.row.updateTime | dateTimeFormat }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column align="center" label="操作" width="260px">
              <template slot-scope="scope">
                <el-button @click="productEdits(scope.row.id)" type="primary" icon="el-icon-edit" size="mini">编辑
                </el-button>
                <el-button @click="productEditsDy(scope.row.id)" type="info" icon="el-icon-monitor" size="mini">打印
                </el-button>
                <el-button @click="productDelete(scope.row.id)" type="danger" icon="el-icon-delete" size="mini">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
      ...
      <el-tab-pane label="产品类型" name="firsts">
        <el-button type="primary" style="marginBottom:15px" @click="productTypeAddId">添加产品类型</el-button>
    ...
    <!-- 打印条形码 -->
    <el-dialog title="条形码" :visible.sync="dialogVisibleDy" width="30%" @close="dialogVisibleDyClose">
      <el-button v-if="supplierDataId" type="primary" size="small" v-print="printObj" style="marginBottom:10px">打印
      </el-button><br>
      <el-select style="marginBottom:10px" v-model="supplierDataId" placeholder="请选择对应供应商" clearable
        @change="userSelectbarcode">
        <el-option v-for="item in supplierData" :key="item.id" :label="item.name" :value="item.id">
        </el-option>
      </el-select>
      <div>
        <!-- <barcode id="printMe" :value="codeDy" :options="barcode_option" tag="svg"></barcode> -->
        <img id="printMe" :src="codeDy" alt="">
      </div>
    </el-dialog>
    <!-- 添加产品分类 -->
    <el-dialog title="添加产品分类" @close="addPartnerCloses" :visible.sync="dialogVisibles" width="20%">
      <el-form :model="addPartners" ref="ruleForms" label-width="120px" class="demo-ruleForm">
        <el-form-item label="产品分类名称" prop="name" :rules="[
            {
              required: true,
              message: '产品分类名称',
              trigger: ['blur', 'change'],
  ...
  export default {
    ...
    methods: {
      ...
      productEditsDy(id) {
        ...
        this.productEditsDyId = id
        // let ip = process.env.VUE_APP_SERVER_BASE_URL
        // console.log(ip);
        this.$network.basic.product
          .productTypeListIdDy(id)
          .then((res) => {
            console.log(res);
            // console.log('data:image/jpeg;base64,' + this.arrayBufferToBase64(res));
            if (res.code === 0) {
              this.supplierData = res.data.companyVOS
              //   // this.codeDy = res.data.code;
              // this.codeDy = 'data:image/jpeg;base64,' + this.arrayBufferToBase64(res)
              //   // console.log(res.data);
            } else {
              this.$notify({
                title: "失败",
      ...
      // 打印弹框消失 删除数据
      dialogVisibleDyClose() {
        // console.log(12);
        this.supplierDataId = ""
        this.codeDy = ""
      },
      // 用户选择供应商  进行条形码的打印
      userSelectbarcode(val) {
        if (val != "") {
          this.$network.basic.product
            .productTypeListIdBingDy(this.productEditsDyId, val)
            .then((res) => {
              // console.log(res);
              // console.log('data:image/jpeg;base64,' + this.arrayBufferToBase64(res));
              // if (res.code === 0) {
