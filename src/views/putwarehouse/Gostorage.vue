  ...
  <div>
          ...
          <el-table :data="gostorageData" border>
            <el-table-column type="expand">
              <template slot-scope="scope">
                <div class="rolesexpand">
                  <div>
                    <el-tag size="mini" type="info">入库编号</el-tag>
                    {{ scope.row.code }}
                  </div>
                  <div>
                    <el-tag size="mini" type="info">经办人</el-tag>
                    {{ scope.row.gestore }}
                  </div>
                  <div>
                    <el-tag size="mini" type="info">入库单id</el-tag>
                    {{ scope.row.id }}
                  </div>
                  <div>
                    <el-tag size="mini" type="info">工具人</el-tag>
                    {{ scope.row.lastOperationUser }}
                  </div>
                  <div>
                    <el-tag size="mini" type="info">采购单编号</el-tag>
                    {{ scope.row.purchaseCode }}
                  </div>
                  <div>
                    <el-tag size="mini" type="info">备注</el-tag>
                    {{ scope.row.remark == null ? "" : scope.row.remark }}
                  </div>
                  <div>
                    <el-tag size="mini" type="info">驳回理由</el-tag>
                    {{
                      scope.row.overruledRemark == null
                        ? ""
                        : scope.row.overruledRemark
                    }}
                  </div>
                  <div>
            ...
            <el-table-column align="center" label="操作" width="230">
              <template slot-scope="scope">
                ...
                <el-popconfirm confirmButtonText="好的" cancelButtonText="不用了" icon="el-icon-info" iconColor="#e3c048"
                  title="确定提交采购订单进行审核吗?" v-if="
                    scope.row.status == 3 ||
                      (scope.row.count !== null && scope.row.status == 1)
    ...
    <el-dialog :title="ids == '' ? '添加入库单' : '修改入库单'" :visible.sync="dialogVisible" @close="gostorageClose" width="23%">
      <el-form :model="ids == '' ? ruleForm : stockInVO" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item v-if="ids == ''" label="入库类型" prop="type" :rules="[
            {
              required: true,
              message: '请选择采购单号',
              trigger: ['blur', 'change']
        ...
        <el-form-item v-else label="入库类型" prop="type" :rules="[
            {
              required: true,
              message: '请选择采购单号',
              trigger: ['blur', 'change']
    ...
    <el-dialog :title="'入库详情' + (stockInVOSCode == '' ? '' : stockInVOSCode)" :visible.sync="dialogVisibleStockInDetail"
      ...
      <el-button :disabled="
          stockInVOStatus == 2
            ...
            ? true
            : false
        " type="primary" size="mini" style="marginLeft:10px" @click="stockInDetailAddSm">扫码添加</el-button>
      <el-card>
        <el-table :data="stockInDetailVOS" border style="width: 100%">
          ...
          <el-table-column align="center" label="所在仓库/建议厂库">
            <template slot-scope="scope">
              {{ scope.row.warehouse }}/{{
                scope.row.adviseWarehouse == null
                  ? "无"
                  : scope.row.adviseWarehouse
              }}
            </template>
          </el-table-column>
          <el-table-column align="center" label="所在库区/建议库区">
            <template slot-scope="scope">
              {{ scope.row.reservoir }}/{{
                scope.row.adviseReservoir == null
                  ? "无"
                  : scope.row.adviseReservoir
              }}
            </template>
          </el-table-column>
          <el-table-column align="center" label="所在货架/建议货架">
            <template slot-scope="scope">
              {{ scope.row.shelves }}/{{
                scope.row.adviseShelves == null ? "无" : scope.row.adviseShelves
              }}
            </template>
          </el-table-column>
          <el-table-column align="center" label="所在层数/建议层数">
            <template slot-scope="scope">
              {{ scope.row.layer }}/{{
                scope.row.adviseLayer == null ? "无" : scope.row.adviseLayer
              }}
            </template>
          </el-table-column>
    ...
    <el-dialog :title="SkuEditId == '' ? '添加' : '编辑'" :visible.sync="dialogVisibleDetail" width="25%"
      ...
      <el-form :model="SkuEditId == '' ? detailAdd : stockInDetailVO" ref="ruleFormDetail" label-width="100px"
        ...
        <el-form-item v-if="SkuEditId == ''" label="产品数量" prop="quantity" :rules="[
            {
              pattern: /^[0-9]*$/,
              message: '请输入正确的数量',
              trigger: ['blur', 'change']
        ...
        <el-form-item v-else label="产品数量" prop="quantity" :rules="[
            {
              pattern: /^[0-9]*$/,
              message: '请输入正确的数量',
              trigger: ['blur', 'change']
    ...
    <!-- 扫码添加 -->
    <el-dialog title="扫码添加" :visible.sync="dialogVisiblesaoma" width="25%" @close="dialogVisiblesaomaClose">
      <el-form :model="arrst" ref="ruleFormDetails" label-width="100px" class="demo-ruleForm">
        <el-form-item label="产品编号">
          <el-input style="width:200px" type="text" v-model="arrst.names" @change="valuesm"></el-input>
        </el-form-item>
        <el-form-item label="产品" prop="skuVOSId">
          <!-- <el-cascader v-model="detailAdd.skuVOSId" style="width:220px" :options="skuVOS" @change="skuVOSChange"
            :props="skuVOSprops"></el-cascader> -->
          <el-input style="width:200px" v-model="skuVOData.skuName" disabled></el-input>
        </el-form-item>
        <el-form-item label="产品单位" prop="name">
          <el-input style="width:100px" disabled v-model="skuVOData.categoryName"></el-input>
        </el-form-item>
        <el-form-item label="选择仓库" prop="skuVOSIdsmao">
          <el-cascader v-model="arrst.skuVOSIdsmao" style="width:280px" :options="warehouseVOS"
            @change="skuVOSIdsChange" :props="warehouseVOSprops">
          </el-cascader>
        </el-form-item>
        <el-form-item label="选择层数" prop="valueceng">
          <el-select v-model="arrst.valueceng" placeholder="请选择" style="width:100px">
            <el-option v-for="(item, index) in arrlayer" :key="index" :label="index + 1 + '层'" :value="index + 1">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="产品数量" prop="quantity" :rules="[
            {
              pattern: /^[0-9]*$/,
              message: '请输入正确的数量',
              trigger: ['blur', 'change']
          ...
          <el-input style="width:120px" v-model="arrst.quantity"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisiblesaoma = false">取 消</el-button>
        <el-button type="primary" @click="detailAddSkussm('ruleFormDetails')">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      // 获取入库列表
      getGostorageList() {
        this.$network.putwarehouse.gostorage
          .gostorageList({
      ...
      // 扫码清除
      dialogVisiblesaomaClose() {
        this.arrst = {};
        this.arrst.skuVOSIdsmao = [];
        this.arrlayer = "";
        this.skuVOData = {};
        this.$refs["ruleFormDetails"].resetFields();
      },
      ...
      // 扫码添加
      detailAddSkussm(ruleFormDetails) {
        this.$refs[ruleFormDetails].validate(valid => {
          if (valid) {
            this.$network.putwarehouse.gostorage
              .stockInDetailAdd({
                stockInId: this.StockInDetailId,
                ...
                warehouseId: this.arrst.skuVOSIdsmao[0],
                reservoirId: this.arrst.skuVOSIdsmao[1],
                shelvesId: this.arrst.skuVOSIdsmao[2],
                layer: this.arrst.valueceng,
                quantity: this.arrst.quantity
              })
              .then(res => {
                // console.log(res);
                if (res.code === 0) {
      ...
      procurementSkuEdit(id) {
        ...
        this.dialogVisibleDetail = true;
        this.$network.putwarehouse.gostorage
          .gostorageDetailEditId(id)
          .then(res => {
            // console.log(res);
            if (res.code === 0) {
              this.stockInDetailVO = res.data.stockInDetailVO;
              let p1 = JSON.parse(
                JSON.stringify(res.data.stockInDetailVO.selectedSku[0])
              );
              ...
              let p3 = JSON.parse(
                JSON.stringify(res.data.stockInDetailVO.selectedShelves[0])
              );
              let p4 = JSON.parse(
                JSON.stringify(res.data.stockInDetailVO.selectedShelves[1])
              );
              let p5 = JSON.parse(
                JSON.stringify(res.data.stockInDetailVO.selectedShelves[2])
              );
